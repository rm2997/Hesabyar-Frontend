...
export const EditCustomer = ({
  ...
  useEffect(() => {
    const loadFormData = async () => {
      ...
      const tmpLocations = [];
      for (const newLocation of customer?.locations) {
        tmpLocations.push({ ...newLocation, unique: makeUniqueKey() });
      }
      setLocations([...tmpLocations]);

      const tmpPhoneNumbers = [];
      for (const newPhone of customer?.phoneNumbers) {
        tmpPhoneNumbers.push({ ...newPhone, unique: makeUniqueKey() });
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await UpdateCustomer(formData.id, {
      ...formData,
      phoneNumbers,
      locations,
    });
